...
class MyAndroid extends PolymerElement {
	static get template() {
		return html `
      ...
      <style include="shared-styles">
        :host {
          ...
          --app-grid-item-height: 100%;
			...
			<div class="banner flexchild flex-vertical green-bg">
				<iron-image class="bg" preload fade sizing="contain" src="../images/assets/projects/android.svg"  alt="Banner"></iron-image>
			</div>
			...
			<template is="dom-repeat" items="[[ajaxResponse0.android]]" as="android">
				<div class\$="[[getUIType(UI)]]">
					<template is="dom-if" if="{{!error0}}">
						<div class="actions flex-justified">
							<div class="title">
								<iron-icon class\$="[[_computeFgClass(android.color)]] big" icon="my-icons:{{android.icon}}"></iron-icon>{{android.title}}<span> ({{android.sub.length}})</span>
							</div>
							<paper-icon-button
									hidden\$="{{!wideLayout}}"
									toggles
				...
				<div class\$="[[getUIType(UI)]] actions flex-center-center">
					<a href="{{android.link}}">
						<paper-button raised class\$="[[_computeBgClass(android.color)]]" aria-label="View all">View all projects<iron-icon icon="my-icons:arrow-forward"></iron-icon></paper-button>
					</a>
				</div>
			</template>
...
window.customElements.define('my-android', MyAndroid);
