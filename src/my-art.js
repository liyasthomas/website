...
class MyArt extends PolymerElement {
  static get template() {
    return html`
      ...
      <iron-ajax
        auto
        url="https://api.tumblr.com/v2/blog/liyasthomas.tumblr.com/posts/photo?api_key=k0Zl9Xz2V8rZ0TiBJmV5mREM9KUEieE0AkAx0cvbKJpbkwxN4p"
        id="ajax0"
        loading="{{loading0}}"
        handle-as="json"
        last-error="{{error0}}"
      ...
      <template is="dom-if" if="{{!error0}}">
        ...
        <div class="grid app-grid" has-aspect-ratio>
          <template
            is="dom-repeat"
            items="[[ajaxResponse0.response.posts]]"
            as="posts"
          >
              ...
              <div class="container">
                <div class="flexchild flex-vertical">
                  <iron-image
                    ...
                    fade
                    sizing="contain"
                    src="{{posts.photos.0.original_size.url}}"
                    alt="{{sub.title}}"
                  ></iron-image>
                </div>
        ...
        <div class="grid actions flex-center-center">
          <a
            href="http://liyasthomas.tumblr.com"
            target="_blank"
            rel="noopener"
          >
            <paper-button class="secondary" raised aria-label="View all"
              >Visit tumblr</paper-button
            >
          </a>
        </div>
      </template>
...
<iron-image class="lightbox" preload fade sizing="contain" src="` +
        event.model.__data.posts.photos[0].original_size.url +
