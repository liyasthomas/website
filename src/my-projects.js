...
class MyProjects extends PolymerElement {
      ...
      <template is="dom-repeat" items="[[ajaxResponse0.web]]" as="web">
          ...
          <div>
            <paper-menu-button horizontal-align="right">
              ...
              <paper-listbox
                ...
                selected="{{sortWebVal}}"
              >
                <paper-icon-item name="date"
                  ><iron-icon
                    icon="my-icons:date-range"
                    slot="item-icon"
                  ></iron-icon
                  >Date<paper-ripple></paper-ripple
                ></paper-icon-item>
                <paper-icon-item name="title"
                  ><iron-icon
                    icon="my-icons:sort-by-alpha"
                    slot="item-icon"
                  ></iron-icon
                  >Alphabet<paper-ripple></paper-ripple
                ></paper-icon-item>
              </paper-listbox>
            </paper-menu-button>
            <paper-icon-button
        ...
        <div class$="[[getUIType(UI)]] app-grid" has-aspect-ratio>
          <template
            ...
            items="[[web.sub]]"
            as="sub"
            filter="{{_filter(filterVal)}}"
            sort="{{_sort(sortWebVal)}}"
            rendered-item-count="{{renderedWebCount}}"
          >
            <a
              href="{{sub.link}}"
        ...
        <div class$="[[getUIType(UI)]] actions flex-center-center">
          <a href="{{web.link}}">
            <paper-button class="secondary" raised aria-label="View all"
              >View all {{web.title}} projects<iron-icon
      ...
      <template is="dom-repeat" items="[[ajaxResponse0.others]]" as="others">
        ...
        <div class$="[[getUIType(UI)]] app-grid" has-aspect-ratio>
          <template
            ...
            as="sub"
            filter="{{_filter(filterVal)}}"
            sort="{{_sort(sortOthersVal)}}"
            rendered-item-count="{{renderedOthersCount}}"
          >
            <a
              href="{{sub.link}}"
              class$="[[_computeTileClass(sub.color)]] item"
        ...
        <div class$="[[getUIType(UI)]] actions flex-center-center">
          <a href="{{others.link}}">
            <paper-button class="secondary" raised aria-label="View all"
              >View all {{others.title}} projects<iron-icon
                icon="my-icons:arrow-forward"
              ></iron-icon
            ></paper-button>
          </a>
  ...
  _filter(val) {
    return sub => {
      ...
      if (!sub) return false;
      return (
        (sub.title && ~sub.title.toLowerCase().indexOf(val.toLowerCase())) ||
        (sub.description &&
          ~sub.description.toLowerCase().indexOf(val.toLowerCase()))
      );
    };
  }
  _sort(val) {
