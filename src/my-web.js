...
class MyWeb extends PolymerElement {
    ...
    return html`
      ...
      <div class="banner flexchild flex-vertical">
        <iron-image
          ...
          fade
          sizing="contain"
          src="../images/assets/feeds/web.svg"
          alt="Banner"
        ></iron-image>
      </div>
      <iron-ajax
        auto
        url="../data/web_feeds.json"
        id="ajax0"
        loading="{{loading0}}"
        handle-as="json"
        last-error="{{error0}}"
      ...
      <template is="dom-repeat" items="[[ajaxResponse0.web]]" as="web">
        ...
        >
          <div class="title">
            {{web.title}} <span>({{renderedCount}})</span>
          </div>
          <div>
        ...
        >
          ...
          <a href="{{web.link}}">
            <paper-button class="secondary" raised aria-label="View all"
              >Back to all projects<iron-icon
  ...
  _getAllResults(event) {
    this.resData = this.data.web[0].sub;
