...
class MyPostwoman extends PolymerElement {
	static get template() {
      ...
      <paper-dialog id="lightbox" class="lightboxdialog" on-click="toggleLightbox"></paper-dialog>
			<div class="banner flexchild flex-vertical">
				<iron-image preload fade sizing="contain" src="../images/assets/projects/postwoman.svg" alt="Postwoman"></iron-image>
			</div>
			...
			<iron-ajax auto url="../data/postwoman_feeds.json" id="ajax0" loading="{{loading0}}" handle-as="json" last-error="{{error0}}" last-response="{{ajaxResponse0}}">
			</iron-ajax>
			...
			<div class$="[[getUIType(UI)]] content">
				<p>Forget all the hassle of working with complicated testing tools. Learn how to test your APIs with ease!</p>
			</div>
			<template is="dom-if" if="{{loading0}}">
				<div class$="[[getUIType(UI)]] actions flex-center-center" hidden$="[[!loading0]]">
					<paper-spinner-lite active$="[[loading0]]"></paper-spinner-lite>
			...
			<div class$="[[getUIType(UI)]] content">
				<p>Join the open source community and contribute to make Postwoman better.</p>
			</div>
			<template is="dom-if" if="{{loading0}}">
				<div class$="[[getUIType(UI)]] actions flex-center-center" hidden$="[[!loading0]]">
					<paper-spinner-lite active$="[[loading0]]"></paper-spinner-lite>
	...
	toggleLightbox(event) {
		...
		let model = (this.$.lightbox.opened) ?
			`
<iron-image class="lightbox" preload fade sizing="contain" src="`+event.model.__data.sub.img+`" alt="Postwoman"></iron-image>
			` :
			`
